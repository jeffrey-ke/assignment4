...
def colours_from_spherical_harmonics(spherical_harmonics, gaussian_dirs):
    ...
    colors = SH_C0 * c0
    dir = gaussian_dirs #N,3
    x = dir[:,0:1] # to keep the dims N,1
    y = dir[:,1:2]
    z = dir[:,2:]
    colors = colors - SH_C1 * y * c1 + SH_C1 * z * c2 - SH_C1 * x * c3
    xx, yy, zz = x * x, y * y, z * z # these will all be shape (N,1)
    xy, yz, xz = x * y, y * z, x * z
    colors = colors +	SH_C2_0 * xy * c4 + \
        SH_C2_1 * yz * c5 + \
        SH_C2_2 * (2.0 * zz - xx - yy) * c6 + \
        SH_C2_3 * xz * c7 + \
        SH_C2_4 * (xx - yy) * c8
                # 1, * N,1 * N,3
                # yes this is correct because I want to an element wise multiplication over all N gaussians.
    colors = colors + \
