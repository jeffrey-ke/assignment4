...
class Gaussians:
    ...
    def compute_cov_3D(self, quats: torch.Tensor, scales: torch.Tensor):
        ...
        if self.is_isotropic:
            R = quaternion_to_matrix(quats) # shape (n,3,3)
            S = torch.eye(3,3, device=quats.device).repeat(N,1).view(N,3,3)
            S *= scales.unsqueeze(-1)
            sigma = torch.matmul(R, S)
            sigma = torch.matmul(sigma, S.transpose(1,2))
            sigma = torch.matmul(sigma, R.transpose(1,2))
            cov_3D = sigma
            ### YOUR CODE HERE ###
            # will quats ever be used? What is isotropic btw... you don't optimize 
            # quats when isotropic. Why? Maybe 
        # HINT: You can use a function from pytorch3d to convert quaternions to rotation matrices.
        else:
            R = quaternion_to_matrix(quats) # shape (n,3,3)
            S = torch.eye(3,3, device=quats.device).repeat(N,1).view(N,3,3)
            S *= scales.unsqueeze(-1)
            # scales is (N,3); S is (N,3,3)
            # if I unsqueeze scales to be (N,3,1), it should work. 
            sigma = torch.matmul(R, S)
            sigma = torch.matmul(sigma, S.transpose(1,2))
            sigma = torch.matmul(sigma, R.transpose(1,2))
            cov_3D = sigma  # (N, 3, 3)
    ...
    def compute_cov_2D(
        ...
        cov_2D = torch.matmul(J, W)  # (N, 2, 2)
        cov_2D = torch.matmul(cov_2D, cov_3D)
        cov_2D = torch.matmul(cov_2D, W.transpose(1,2))
        cov_2D = torch.matmul(cov_2D, J.transpose(1,2))
    ...
    def compute_means_2D(means_3D: torch.Tensor, camera: PerspectiveCameras):
        ...
        batchify = lambda t : t.unsqueeze(0)
        unbatchify = lambda t : t.squeeze(0)
        means_2D = camera.transform_points_screen(batchify(means_3D))
        means_2D = unbatchify(means_2D)[:,:2]  # (N, 2)
        # transform the means3d to the perspective of the camera and then 
        # do a transform that puts them in scren space and take out the z
        # coordinate
        return means_2D
...
class Scene:
    ...
    def compute_alphas(self, opacities, means_2D, cov_2D, img_size):
        ...
        ### YOUR CODE HERE ###
        # HINT: Can you find a function in this file that can help?
        # all my power is super negative
        power = self.gaussians.evaluate_gaussian_2D(points_2D, means_2D, cov_2D_inverse).double()  # (N, H*W)
